...
struct VERTEX {FLOAT X, Y, Z; gbVec3 Normal;};
...
void RenderFrame(void)
{
    ...
    gbMat4 matRotate, matView, matProjection;
    gbMat4 matFinal;
    ...
    gb_mat4_look_at(&matView,
                    {0.0f, 3.0f, 5.0f},   // the camera position
                    {0.0f, 0.0f, 0.0f},    // the look-at position
                    {0.0f, 1.0f, 0.0f});   // the up direction
    
    // create a projection matrix
    gb_mat4_perspective(&matProjection,
                        (FLOAT)gb_to_radians(45),                    // field of view
...
void InitPipeline()
{
    ...
    if(S_OK != dev->CreateVertexShader(g_VShader, sizeof(g_VShader), 0, &pVS))
        int i = 0; 
    
    if(S_OK != dev->CreatePixelShader(g_PShader, sizeof(g_PShader), 0, &pPS))
        int i = 0;
    ...
    dev->CreateInputLayout(ied, 2, g_VShader, sizeof(g_VShader), &pLayout);
